...
#include <QDomElement>
#include <QDomNode>
class pencereGorevler:public QWidget ,public Ui::FormGorevler
{
 Q_OBJECT
public:
...
// cBDoldur();
     ...
     sorgu_model->setQuery("SELECT * FROM gorevler");
...
tablo->hideColumn(0);
  ...
  mapper->addMapping(LeGorevYeri, 1);
...
void Guncelle()
{
   ...
   if(mod_guncelle()){
	...
	gorevler *uye = new gorevler(tmp_id,LeGorevYeri->text());
        uye->Guncelle();
		sorgu_model->setQuery("SELECT * FROM gorevler");
		sorgu_model->query();
   }
...
if(mod_kayit()){
gorevler *uye = new gorevler(LeGorevYeri->text());

uye->Kaydet();
sorgu_model->setQuery("SELECT * FROM gorevler ORDER by id DESC");
sorgu_model->query();
}
}
...
void Sil()
{
 ...
 gorevler *uye = new gorevler(tmp_id,0);
	uye->Sil();
		sorgu_model->setQuery("SELECT * FROM gorevler");
		sorgu_model->query();
